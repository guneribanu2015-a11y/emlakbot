...
def haber_cek(sorgu, etiket, kategori, max_haber=5):
...
@st.cache_data(ttl=1800)
def tum_haberleri_cek(max_haber_per_kaynak=5):
    sonuc = []
    for k in KAYNAKLAR:
        sonuc.extend(haber_cek(k["sorgu"], k["etiket"], k["kategori"], max_haber_per_kaynak))
    return sonuc
...
def haber_karti(haber):
    ...
    with st.container():
        ...
        with col1:
            st.markdown(f"**[{haber['baslik']}]({haber['link']})**")
        with col2:
...
def haber_bolumu_goster(max_haber_per_kaynak=5):
